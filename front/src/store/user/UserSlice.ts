	...
	extraReducers(builder) {
		builder
			...
			.addCase(UserService.usernameCheck.rejected, (state, action) => {
				state.isLoading.loginCheck = false;
				state.error.loginCheck = String(Array.isArray(action.payload) ? action.payload.join('. ') : action.payload);
				state.user = null;
			})
			...
			.addCase(UserService.recovery.rejected, (state, action) => {
				state.isLoading.recovery = false;
				state.error.recovery = String(Array.isArray(action.payload) ? action.payload.join('. ') : action.payload);
				state.user = null;
			});
	},
});
