...
export class ChatRoleRepository {
	constructor(
		...
		private readonly chatModel: Model<ChatRole>,
	...
	public async createChatRole(roleDto: ChatRole): Promise<ChatRole> {
		...
		try {
			const chatRole = await this.chatModel.findOne({ chat: roleDto.chat, name: roleDto.name || DEFAULT_CHAT_ROLE_NAME }).lean();

			if (chatRole) {
				throw new HttpException('Role with this name already exist in this chat', HttpStatus.BAD_REQUEST);
			}

			return await this.chatModel.create(roleDto);
		} catch (error) {
